...
predict_results_path = os.getenv(
    ...
    os.getenv('MOBILE_PREDICT_RESULTS', 'predict_results.csv'),
)
...
if os.path.exists(stats_path):
    ...
    for obj in records:
        ...
        if sessA not in probs:
            continue
        if sessA not in meta:
            ip = obj.get('ip')
            meta[sessA] = str(ip) if ip else ''
        if sessA not in user_agents:
            ua = obj.get('ua', '')
            user_agents[sessA] = str(ua) if ua else ''
else:
